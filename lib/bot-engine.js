...
- Address the recipient by their FIRST NAME ONLY (e.g., "Hi ${firstName}"). 
- STRICTLY FORBIDDEN: "sir", "madam", "mam", "ma'am", "Mr.", "Ms.", "Mrs.", or ANY gendered honorific — do NOT use them anywhere in the note. First name alone is the correct professional greeting on LinkedIn.
- Do NOT guess the recipient's gender
...
Return ONLY the note text, nothing else.`;
  ...
  personalizeNote(name, headline = '') {
    ...
    // Gender guessing unreliable (lady ni "sir", gentleman ni "madam" pilavakunda) —
    // {name} ippudu first name matrame istundi, sir/madam lekunda.
    let note = this.config.connectionNote;
    if (note.includes('{name}')) {
      note = note.replace(/\{name\}/g, firstName);
    }
    if (note.includes('{honorific}')) {
      note = note.replace(/\{honorific\}/g, '');
    }
  ...
  async connectViaProfilePage(browser, profileUrl, personName, headline = '', role = '') {
    ...
    try {
      ...
      if (vanityMatch) {
        ...
        if (!profilePage.url().includes('/login') && !profilePage.url().includes('/authwall')) {
          const fastResult = await this.handleConnectionModal(profilePage, personName, headline, role);
          if (fastResult) {
            await this.sleep(1000);
            await profilePage.close();
            return fastResult;
