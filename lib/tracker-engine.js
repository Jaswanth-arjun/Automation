...
export async function searchLiveConnections(keyword) {
  ...
  try {
    ...
    // Modern LinkedIn Search URL with 1st degree network filter
    ...
    const results = await page.evaluate(() => {
      const list = [];
      const items = Array.from(document.querySelectorAll('.reusable-search__result-container, li.reusable-search__result-container, div.entity-result, div[data-chameleon-result-urn]'));
      items.forEach((item) => {
