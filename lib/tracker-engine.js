...
export async function searchLiveConnections(keyword, existingBrowser = null) {
  ...
  try {
    ...
    console.log(`[RAG Live Search] Page title: "${pageTitle}"`);

    if (pageTitle.toLowerCase().includes('sign in') || pageTitle.toLowerCase().includes('log in') || pageTitle.toLowerCase().includes('auth')) {
      console.warn('[RAG Live Search] ⚠️ User not logged in to LinkedIn in active Chrome session');
      await page.close();
    ...
    // Scroll down to trigger lazy loading of search result cards
    ...
    const results = await page.evaluate(() => {
      ...
      const items = Array.from(document.querySelectorAll(
        '.reusable-search__result-container, li.reusable-search__result-container, div.entity-result, div[data-chameleon-result-urn], .search-results-container li'
      ));

      items.forEach((item) => {
        const titleEl = item.querySelector('.entity-result__title-text a, a[href*="/in/"]');
        const headlineEl = item.querySelector('.entity-result__primary-subtitle, div.entity-result__primary-subtitle');
        const secondaryEl = item.querySelector('.entity-result__secondary-subtitle, div.entity-result__secondary-subtitle');

        if (titleEl) {
          const span = titleEl.querySelector('span[aria-hidden="true"]');
          let name = span ? span.textContent.trim() : titleEl.textContent.trim();
          name = name.replace(/View\s+.*'s\s+profile/i, '').replace(/1st|2nd|3rd\+?/gi, '').replace(/[\n\r]+/g, ' ').trim();
          const url = titleEl.href ? titleEl.href.split('?')[0] : '';

          if (name && !name.toLowerCase().includes('linkedin member') && name.length > 2) {
            list.push({
              name,
              url,
              headline: headlineEl ? headlineEl.textContent.replace(/[\n\r]+/g, ' ').trim() : '',
              location: secondaryEl ? secondaryEl.textContent.replace(/[\n\r]+/g, ' ').trim() : '',
            });
          }
        }
      });
    ...
    console.log(`[RAG Live Search] Successfully found ${results.length} live 1st-degree connections for "${keyword}"`);
    ...
    console.error(`[RAG Live Search] Error during live search: ${err.message}`);
    if (shouldCloseBrowser && browser) {
      try { await browser.close(); } catch {}
    }
    return [];
